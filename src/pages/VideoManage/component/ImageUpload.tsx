...
import { uploadImg } from 'services/oss';
...
const ImageIcon = memo(() => (
  <svg
    ...
    data-spm-anchor-id='a313x.search_index.0.i2.30223a81anm0Mu'
    width='25'
    height='25'
  >
    <path
...
export const ImageTrimmer = memo((props: ImageTrimProps) => {
  ...
  useEffect(() => {
    resizeObserver.current = new ResizeObserver(() => {
      ...
      console.log(parentWidth, parentHeight, originalSize.current.width, originalSize.current.height);
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      ...
      props.onCropChange(
        0,
        0,
        (cropper.offsetWidth * imageNaturalSize.current.width) / parentWidth,
        (cropper.offsetHeight * imageNaturalSize.current.height) / parentHeight,
      );
    };
  ...
  const handleMoveCropper = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const deltaX = Math.max(
        ...
        Math.min(parentWidth - cropper.offsetWidth + moveOrigin.current.x, mouseX),
      );
      const deltaY = Math.max(
        moveOrigin.current.y,
        Math.min(parentHeight - cropper.offsetHeight + moveOrigin.current.y, mouseY),
      );
  ...
  const handleResizeCropper = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
  ...
  const cropperMouseMove = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      e.stopPropagation();
  ...
  const cropperMouseDown = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      e.stopPropagation();
  ...
  const cropperCornerMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    movingCornerIndex.current = parseInt(target.dataset.index ?? '-1', 10);
  }, []);
  ...
  return (
    <div
      className='moe-video-image-trimmer'
    ...
    >
      ...
      <div className='moe-video-image-trimmer-cropper' onMouseDown={cropperMouseDown} ref={cropperRef}>
        <div className='moe-video-image-trimmer-cropper-top-left' data-index={0} onMouseDown={cropperCornerMouseDown} />
        <div
          className='moe-video-image-trimmer-cropper-top-right'
          data-index={1}
          onMouseDown={cropperCornerMouseDown}
        />
        <div
          className='moe-video-image-trimmer-cropper-bottom-left'
          data-index={2}
          onMouseDown={cropperCornerMouseDown}
        />
        <div
          className='moe-video-image-trimmer-cropper-bottom-right'
          data-index={3}
          onMouseDown={cropperCornerMouseDown}
        />
      </div>
...
const ImageUpload = memo((props: { defaultImageUrl?: string; onChange?: (file: File) => void }) => {
  const { defaultImageUrl, onChange } = props;
  ...
  return (
    <>
      <div
        className='moe-video-image-upload-page-container'
      ...
      >
        <input
          ...
          onChange={handleFileChange}
        />
        <button className='moe-video-image-upload-page-button' onClick={handleClick}>
          <ImageIcon />
          <span>上传图片</span>
        </button>
      </div>
