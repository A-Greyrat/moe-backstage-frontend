...
import VideoPlayer, { TripleSpeedForward } from 'mika-video-player';
import { Button, showMessage } from '@natsume_shiki/mika-ui';
import { uploadFileToOss } from 'services/oss';
...
const VideoUpload = memo((props: { setLink: (link: string) => void }) => {
  const { setLink } = props;
    ...
    return (
      <>
        <div className='moe-video-upload-container-loaded'>
          ...
          <Button onClick={handleClick}>点击重新上传</Button>
          {!uploading && <Button onClick={handleUpload}>确认上传</Button>}
          {uploading && (
  ...
  return (
    <div
      className='moe-video-upload-container'
